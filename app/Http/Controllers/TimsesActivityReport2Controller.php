...
class TimsesActivityReport2Controller extends Controller
{
    ...
    public function index()
    {
        ...
        $programtimsesactivity = FinancialFlow::where('financial_flow.data_state', '=', 0)
        ->join('core_timses', 'core_timses.timses_id', '=', 'financial_flow.timses_id')
        ->where('financial_flow.financial_category_id', '=', 8)
        ->where('financial_flow.financial_category_type', '=', 2)
        ->where('financial_flow.program_id', '!=', 0)
        ->where('financial_flow.timses_id', '!=', 0)
        ...
        //     $programtimsesactivity   = $programtimsesactivity->where('timses_id', $timses_id);
        // }
        $programtimsesactivity   = $programtimsesactivity->get();

        return view('content/TimsesActivityReport2_view/ReportTimsesActivity', compact('programtimsesactivity', 'start_date', 'end_date', 'listcoretimses'));
    }

    public function filterTimsesActivityReport2(Request $request)
    {
        ...
        return redirect('/report-timses-activity2');
    }

    public function filterResetTimsesActivityReport2()
    {
    ...
    public function printTimsesActivityReport2()
    {
        if(!Session::get('start_date')){
            $start_date     = date('Y-m-d');
        }else{
        ...
        <table cellspacing=\"0\" cellpadding=\"2\" border=\"0\">
            <tr>
                <td><div style=\"text-align: center; font-size:14px; font-weight: bold\">LAPORAN KEGIATAN TIMSES</div></td>
            </tr>
            <tr>
        ...
        <table cellspacing=\"0\" cellpadding=\"1\" border=\"1\" width=\"100%\">
            <tr>
                ...
                <th width=\"20%\" ><div style=\"text-align: center; font-weight: bold\">Nama Anggota Timses</div></th>
                <th width=\"17%\" ><div style=\"text-align: center; font-weight: bold\">Nama Kegiatan</div></th>
                <th width=\"25%\" ><div style=\"text-align: center; font-weight: bold\">Deskripsi</div></th>
                <th width=\"20%\" ><div style=\"text-align: center; font-weight: bold\">Dana</div></th>
            </tr>
        ";
    ...
    public function exportTimsesActivityReport2()
    {
        if(!Session::get('start_date')){
            $start_date     = date('Y-m-d');
        }else{
        ...
        if(count($programtimsesactivity)>=0){
            ...
            $sheet->setCellValue('B1',"Laporan Kegiatan Timses");	
            ...
            $sheet->setCellValue('D4',"Nama Anggota Timses");
            $sheet->setCellValue('E4',"Nama Kegitan");
            $sheet->setCellValue('F4',"Deskripsi");
            $sheet->setCellValue('G4',"Dana");
            ...
            foreach($programtimsesactivity as $key=>$val){
                ...
                if(is_numeric($key)){
                    ...
                    $sheet = $spreadsheet->getActiveSheet(0);
                    $spreadsheet->getActiveSheet()->setTitle("Laporan Kegiatan Timses");
                    ...
                    $sheet->setCellValue('D'.$j, $this->getTimsesName($val['timses_id']));
