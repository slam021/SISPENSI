        ...
        <div class="card-body">
            <div class="row form-group">
                <div class="col-md-4">
                    ...
                    <input class="form-control input-bb" type="hidden" name="timses_id" id="timses_id" value="{{$timses_id}}" onChange="function_elements_add(this.name, this.value);" autocomplete="off" />
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="" style='text-align:center'>
                            {{-- <a type="button" class="badge bg-success" data-toggle='modal' data-target='#exampleModal'><i class='fas fa-user-circle'></i> Buat Akun</a> --}} <?php
                                if($val['user_id'] == null){
                                    echo "<a type='button' class='badge bg-success' href='".url('/timses/add-account-member/'.$timses_id.'/'.$val['timses_member_id'])."' title='Edit Acara'><i class='fas fa-user-circle'></i> Buat Akun</a>";
                                }else{
                                    // echo "<button disabled type='button' class='badge bg-info' title='Edit Acara'><i class='fas fa-edit'></i> Buat akun</button>";
                                }
                            ?>
                            {{-- <a type="button" class="badge bg-success" href="{{url('/timses/add-account-member/'.$timses_id.'/'.$val['timses_member_id']) }}" title="Hapus"><i class='fas fa-user-circle'></i> Buat Akun</a> --}}
                            <a type="button" class="badge badge-danger" href="{{url('/timses/delete-timses-member/'.$timses_id.'/'.$val['timses_member_id']) }}" title="Hapus"><i class='far fa-trash-alt'></i> Hapus</a>
                        </td>
                    </tr>
