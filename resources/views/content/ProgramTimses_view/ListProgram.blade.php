@inject('Program', 'App\Http\Controllers\ProgramTimsesController')
...
{{-- <form  method="post" action="{{ route('filter-program2') }}" enctype="multipart/form-data">
    @csrf
...
<div class="card border border-dark">
    <div class="card-header bg-dark clearfix">
        ...
        <div class="form-actions float-right">
            <button onclick="location.href='{{ url('program-timses/add') }}'" name="add" class="btn btn-sm bg-cyan" title="Add Data"><i class="fa fa-plus"></i> Tambah Acara</button>
        </div>
    </div>
                        ...
                        <td class="" style='text-align:left'>
                            <?php
                                if($val['program_status'] == 0){
                                    echo "<a type='button' class='badge bg-warning' href='".url('/program-timses/edit/'.$val['program_id'])."' title='Edit Acara'><i class='fas fa-edit'></i> Edit</a>";
                                }else{
                                    
                                }
                            ?>
                            <a type="button" class="badge bg-lime" href="{{ url('/program-timses/detail/'.$val['program_id'])}}" title="Detail Acara"><i class='fas fa-list-ul'></i> Detail</a>
                            <a type="button" class="badge bg-indigo" href="{{ url('/program-timses/documentation-program/'.$val['program_id'])}}" title="Dokumentasi Acara"><i class='far fa-image'></i> Dokumentasi</a>
                            <a type="button" class="badge bg-danger" href="{{ url('/program-timses/delete-program/'.$val['program_id']) }}" title="Hapus Acara"><i class='far fa-trash-alt'></i> Hapus</a>
                        </td>
                    </tr>
